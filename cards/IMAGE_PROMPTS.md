...
실제 얼굴을 닮지 않은 창작 인물. 멀티버스 영웅 그림(`multiverse/art/yunchan-idle.webp`, `yungeon-idle.webp`)을 첫 참고 그림으로, 가족 카드 원화를 그림체 참고로 넣어 Vertex `gemini-3-pro-image`로 그렸다(`tools/vertex-sprite-art.py`).

- `yunchan.png`: 윤찬(10세) — 짧고 단정한 검은 머리. 곤충 박사: 반딧불이 날리는 여름 숲에서 잠자리채와 채집통을 들고 장수풍뎅이·사슴벌레를 자랑. 검정·하늘색 탐험 조끼. 1024×1536, 글자·숫자·로고 없음.
- `yungeon.png`: 윤건(7세) — 동그란 얼굴, 복슬한 갈색 바가지 머리. 축구 선수: 노랑·주황 축구복으로 달밤 축구장에서 불꽃 슛, 뒤에 골대. 1024×1536, 글자·숫자·로고 없음.
...
## 쓰구미의 장난 부하 — 태뿔·찬뿔·건뿔 (2026-09-25)

재윙처럼 아이들의 귀여운 악마 모드(작은 빨간 뿔·보라 박쥐 날개·하트 꼬리). 각자 카드 원화를 첫 참고 그림, 재윙 원화를 둘째 참고 그림으로 Vertex `gemini-3-pro-image`로 그렸다. 무섭지 않고 얄밉게.

- `taeppul.png`: 꼴통 태오 — 보라·빨강 악마 무늬 도복, 달밤 궁전 무도회장에서 유리 구두에 방귀 쿠션을 넣으며 구린 줄무늬 양말을 흔든다. 1024×1536, 글자 없음.
- `chanppul.png`: 번개맨 차니 — 검정·하늘색 번개 슈트와 하늘색 망토, 폭풍 치는 그리스 신전에서 양손으로 하늘색 번개를 쏘아 석상 머리카락을 쭈뼛 세운다. 기존 TV 영웅을 따라 하지 않은 창작 의상. 1024×1536, 글자 없음.
- `geonppul.png`: 헐크 거니 — 초록빛 근육 모습으로 부풀어 찢어진 노랑 셔츠·보라 반바지, 노을 진 세 나라 전장에서 나무 수레를 머리 위로 번쩍 든다. 얼굴은 아이 그대로. 1024×1536, 글자 없음.
