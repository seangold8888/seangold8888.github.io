...
async function main() {
  ...
  try {
    ...
    for(const viewport of [{width:820,height:1180},{width:1180,height:820},{width:390,height:844}]) {
      ...
      await p.reload();await p.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===87);
      await mapContinue(p);
      ...
      for(let step=0;step<280;step++){
        ...
        if(state && state.chapter===7 && state.phase==="restore"){
          ...
          if(state.endingScene===2 && !reloaded){
            await q.reload();await q.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===87);
