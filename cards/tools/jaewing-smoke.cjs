...
(async()=>{
 ...
 try{
  for(const viewport of [{width:390,height:844},{width:820,height:1180},{width:1180,height:820}]){
   ...
   await page.locator('#cardDetailCard img').evaluate(async img=>{await img.decode();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));});
   await page.screenshot({path:path.join(output,viewport.width+'-detail.png')});
   ...
   for(const [i,name] of ['코딱지 튕기기','구름사다리 매달리기','바이킹 그네 밀기'].entries()){
    ...
    await page.locator('#playerCardSlot img').evaluate(async img=>{await img.decode();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));});
    if(i===0)await page.screenshot({path:path.join(output,viewport.width+'-battle.png')});
