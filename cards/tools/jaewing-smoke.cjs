...
(async()=>{
 ...
 try{
  for(const viewport of [{width:390,height:844},{width:820,height:1180},{width:1180,height:820}]){
   ...
   assert.match(await page.locator('#cardDetailStatus').innerText(),/2일.*10문제/);assert.equal(await page.locator('#detailSelectButton').isDisabled(),true);
   assert.equal(await page.locator('#detailUnlockLink').getAttribute('href'),'../math/');
   assert.match(await page.locator('#cardDetailCard').innerText(),/재이의 괴물 친구/);
   await page.locator('[data-detail-close]').first().click();
   await page.evaluate(()=>{const days={};for(let i=0;i<2;i++){const d=new Date();d.setDate(d.getDate()-i*2);days[d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')]=1;}localStorage.setItem('math10_state',JSON.stringify({cardStudyDays:days,garden:10}));});
   ...
   for(const [i,name] of ['날개 툭','장난 바람','재윙 회오리'].entries()){
