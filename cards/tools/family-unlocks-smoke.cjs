...
(async()=>{
 ...
 try{
  ...
  await battle.goto(base+"/cards/");await battle.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===90);
  ...
  for(const viewport of [{width:820,height:1180},{width:1180,height:820},{width:390,height:844}]){
   ...
   await page.goto(base+"/cards/");await page.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===90);
   ...
   for(const [id,goal] of Object.entries(data.familyUnlockGoals).sort((a,b)=>a[1].studyDays-b[1].studyDays)){ // 약한 목표부터: 앞 단계 기록이 뒤 카드를 미리 열지 않게
    for(const [days,problems,locked] of [[goal.studyDays-1,goal.problems,true],[goal.studyDays,goal.problems-1,true],[goal.studyDays,goal.problems,false]]){
     ...
     await page.reload();await page.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===90);
   ...
   await page.waitForFunction(()=>document.querySelectorAll(".card-gallery-item").length===90);
