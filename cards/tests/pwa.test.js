...
test("all 75 collection cards have an install-time webp", () => {
  ...
  assert.equal(sw.CARD_ART_FILES.length, 76);
  assert.equal(new Set(sw.CARD_ART_FILES).size, 76);
...
test("cache generation v88 preserves exact v47 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v88");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v88-/);
  ...
  for (const asset of ["styles.css", "campaign.css", "engine.js", "audio.js", "card-view.js", "vfx-recipes.js", "story-gates.js", "campaign.js", "campaign-ui.js", "combat-cinema.js", "app.js"]) {
    assert.ok(sw.CORE_SHELL.some(entry => entry.endsWith(asset + "?v=47")), asset);
  }
