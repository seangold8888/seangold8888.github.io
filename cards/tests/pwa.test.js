...
test("all 75 collection cards have an install-time webp", () => {
  ...
  assert.equal(sw.CARD_ART_FILES.length, 75);
  assert.equal(new Set(sw.CARD_ART_FILES).size, 75);
...
test("cache generation v82 preserves exact v42 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v82");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v82-/);
  ...
  for (const asset of ["styles.css", "engine.js", "audio.js", "card-view.js", "vfx-recipes.js", "story-gates.js", "app.js"]) {
    assert.ok(sw.CORE_SHELL.some(entry => entry.endsWith(asset + "?v=42")), asset);
  }
