...
test("all 71 collection cards have an install-time webp", () => {
  const data = JSON.parse(fs.readFileSync(path.join(siteRoot, "cards", "cards.json"), "utf8"));
  assert.equal(data.collection.length, 71);
  assert.equal(sw.CARD_ART_FILES.length, 71);
  assert.equal(new Set(sw.CARD_ART_FILES).size, 71);
...
test("cache generation v81 preserves exact v41 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v81");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v81-/);
  ...
  for (const asset of ["styles.css", "engine.js", "audio.js", "card-view.js", "vfx-recipes.js", "story-gates.js", "app.js"]) {
    assert.ok(sw.CORE_SHELL.some(entry => entry.endsWith(asset + "?v=41")), asset);
  }
