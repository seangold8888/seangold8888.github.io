...
test("all 75 collection cards have an install-time webp", () => {
  ...
  assert.equal(sw.CARD_ART_FILES.length, 85);
  assert.equal(new Set(sw.CARD_ART_FILES).size, 85);
...
test("cache generation v114 preserves exact v63 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v114");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v114-/);
  ...
  assert.ok(sw.OPTIONAL_SHELL.includes("./princess/studio.js?v=43"));
  ...
  for (const id of "snow cinder rapunzel mermaid thumb kongjwi briar moon frost sahara lotus sunny".split(" ")) {
    const asset="./princess/assets/heads-v43/"+id+".webp";
    assert.ok(sw.OPTIONAL_SHELL.includes(asset));
    assert.ok(fs.existsSync(path.join(siteRoot,asset)));
    assert.equal(sw.OPTIONAL_SHELL.filter(item=>item===asset).length,1);
  }
  ...
  for (const asset of ["styles.css", "campaign.css", "engine.js", "audio.js", "card-view.js", "vfx-recipes.js", "story-gates.js", "campaign.js", "campaign-ui.js", "combat-cinema.js", "app.js"]) {
    assert.ok(sw.CORE_SHELL.some(entry => entry.endsWith(asset + "?v=63")), asset);
  }
