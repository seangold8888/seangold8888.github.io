...
test("cache generation v147 preserves exact v68 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v147");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v147-/);
...
test("big media and saved stories live in caches that survive a deploy", () => {
  assert.match(sw.STATIC_CACHE, /^adventure-box-v147-static$/);
