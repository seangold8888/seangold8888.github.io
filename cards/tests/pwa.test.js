...
test("cache generation v160 preserves exact v70 card assets and canonical navigation aliases", () => {
  assert.equal(sw.CACHE_VERSION, "v160");
  assert.match(sw.STATIC_CACHE, /^adventure-box-v160-/);
...
test("big media and saved stories live in caches that survive a deploy", () => {
  assert.match(sw.STATIC_CACHE, /^adventure-box-v160-static$/);
