...
test('Jaewing appends exactly one card and preserves all 84 existing cards and their order',()=>{
 assert.equal(data.cards.length,90);assert.equal(data.collection.length,90);assert.equal(data.cards[84].id,'jaewing');assert.equal(data.collection[84],'jaewing');
...
test('all 570 legacy sound plans are unchanged; Jaewing moves route to flick, metal and air',()=>{
 ...
 assert.deepEqual(plansNew.map(p=>p.material),['flick','metal','air']);for(const plan of plansNew)assert.ok(plan.tailMs<=1000);
 assert.ok(!data.cards.slice(0,84).some(card=>card.attacks.some(a=>['🪜','🎢'].includes(a.vfx.emoji))));
});
test('Jaewing is a strong prank villain with safe mechanics, crop, earned math goal and one offline portrait',()=>{
 ...
 assert.match(c.lore,/재이의 귀여운 악마 모드/);assert.equal(c.unlock,'game:math/streak7');
