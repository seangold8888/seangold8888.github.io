...
test("컬렉션 해금 경제는 오디세이 확장 75장 전체를 노출한다", () => {
  ...
  assert.equal(new Set(data.collection).size, 90);
  assert.deepEqual(
...
test("페르세우스 설명과 실제 v1 대전 상대 풀이 레어도 ±1 계약을 지킨다", () => {
  ...
  assert.equal(featuredCards.length, 90, "컬렉션 전원이 대전 가능해야 한다");
...
test("늑대가 약화를 반복해도 인어공주 AI 대전은 무피해 교착 없이 끝난다", () => {
  ...
  assert.ok(state.winner, "AI 대전은 승패가 나야 한다");
  assert.ok(actions < 70, "레어도 ±1 실제 대진은 3분 상한 안에 끝나야 한다");
  assert.ok(state.sides.player.hp <= 0 || state.sides.enemy.hp <= 0);
});
...
test("검수 완료된 24장 PNG·WebP 원화는 바뀌지 않는다", () => {
  ...
  // 기존 검수 해시는 재계산하지 않는다. G1~G4 파일은 별도 검사한다.
  const files = data.collection
    .filter((id) => ![
      ...
      "jaewing", "yunchan", "yungeon", "taeppul", "chanppul", "geonppul",
    ].includes(id))
