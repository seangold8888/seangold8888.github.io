...
test("G1 stays byte-for-byte stable after G2", () => {
  const g1Data = data.cards.filter(card => !g2Ids.includes(card.id));
  const g1Collection = data.collection.filter(id => !g2Ids.includes(id));
...
test("G2 adds exactly four playable monster cards and preserves all 28 earlier cards", () => {
  ...
  assert.deepEqual(data.collection.slice(-4), g2Ids);
  const earlier = data.cards.filter(card => !g2Ids.includes(card.id));
  ...
  assert.deepEqual(data.cards.reduce((counts, card) => { counts[card.type] = (counts[card.type] || 0) + 1; return counts; }, {}), {brave: 6, wise: 6, magic: 10, monster: 10});
...
test("local preview can unlock G1 and G2, but a public preview query cannot bypass listening", () => {
  for (const id of ids) {
...
test("G1 and G2 have 40 independent questions, all traceable to the accepted matching-story bank", () => {
...
test("G1 28장의 PNG·WebP 56개 배포 원화는 검증된 매니페스트와 일치한다", () => {
  const files = data.collection.filter(id => !g2Ids.includes(id))
