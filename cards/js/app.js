(function () {
  ...
  function cacheDom() {
    [
      ...
      "selectionDock", "selectedPortrait", "selectedName", "battleButton",
  ...
  function updateSelectionDock() {
    ...
    dom.battleButton.disabled = false;
  }
  ...
  function openLockedDialog(card) {
    ...
    dom.lockedDescription.textContent = "「" + storyName + "」를 끝까지 들으면 이 영웅과 함께 대결할 수 있어요.";
  ...
  function pickEnemy() {
    ...
    const requestedCard = cards.find(function (card) {
      return card.id === requested && (!selectedCard || card.id !== selectedCard.id);
    });
    if (isLocalQaHost() && requestedCard) return requestedCard;

    const pool = window.CardEngine.getBalancedEnemyPool(cards, selectedCard);
    return pool[Math.floor(Math.random() * pool.length)] || selectedCard || cards[0];
  }
  ...
  function isSupportedAttack(attack) {
    return attack && attack.v1 !== false && !(attack.fx || "").startsWith("v2_");
  }
  ...
  async function init() {
    ...
    try {
      ...
      cards = FEATURED_IDS.map(function (id) {
        return data.cards.find(function (card) { return card.id === id; });
      }).filter(function (card) {
        return card && card.v1 !== false &&
          Array.isArray(card.attacks) && card.attacks.some(isSupportedAttack);
      });
      const requested = new URLSearchParams(location.search).get("card");
      selectedCard = cards.find(function (card) {
        return card.id === requested && isUnlocked(card);
      }) || cards.find(isUnlocked) || null;
      renderCollection();
      if (new URLSearchParams(location.search).get("battle") === "1" && selectedCard) {
        startBattle();
      }
    } catch (error) {
