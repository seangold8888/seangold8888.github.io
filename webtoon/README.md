...
| `words.js` | 우리 집 말 모음. 태오어 사전(`TAEO_WORDS`), 태오가 매일 하는 말(`TAEO_SAYINGS`), 재이가 아기 때 한 말(`JAEI_WORDS`). 목록 화면과 매 화 끝 ‘오늘의 태오어’에 나온다. |
...
`words.js`의 알맞은 목록 맨 아래에 한 줄을 더한다. 태오어 사전과 재이 말은 `say`(한 말), `real`(원래 말), `what`(설명), 태오 입버릇은 `say`, `when`.
