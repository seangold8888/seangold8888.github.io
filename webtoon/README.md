...
| `taeo-words.js` | 태오어 사전. 태오가 태오식으로 부르는 말 모음(목록 화면과 매 화 끝 ‘오늘의 태오어’에 나온다). |
...
`taeo-words.js`의 목록 맨 아래에 한 줄을 더한다. `say`는 태오가 하는 말, `real`은 원래 말, `what`은 짧은 설명.

```js
{ say: "쓰구미", real: "익스큐즈미", what: "실례합니다 (영어)" },
```

더한 뒤 `index.html`과 `sw.js`의 `taeo-words.js?v=` 숫자를 함께 올린다.
