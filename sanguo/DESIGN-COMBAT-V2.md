...
## M1 검증 기록 — 2026-09-06 (클로드)

대상: `codex/sanguo-combat-v2-m1` 커밋 `89cae92` 빌드. 방법: Playwright, 워크트리 정적 서빙, **20fps 이상 확보 후** 측정(640×400 뷰포트). 8fps 헤드리스에서는 `BUFFER_MS 240` 만료로 정상 입력도 유실되어 결과 무효 — 아이패드 60fps에선 무관.

| 기준 | 결과 | 근거 |
|---|---|---|
| A1 4행동 매핑 | 통과 | J 3연타→체인 3, K 탭 등록, L→필살 배너 "청룡일섬", F→승마 배너. 터치 공격 3연타→체인 3 |
| A2 캔버스 탭 구역 제거 | 통과 | 옛 우측 구역 터치 20회+마우스 20회 → 체인 0. 옛 좌측 구역 700ms 홀드 → 지면 이동 0 (양성대조 A키 = 76px) |
| A3 탭/홀드 분리 | 통과 | 키보드·터치 모두 600ms 홀드 → 정확히 1회, 탭+홀드 → 2 (중복 발동 없음) |
| 옛 키 비활성 | 통과 | E 3연타 → 체인 0 |
| B1 호로관 그림 배경 | 통과 | 가로·세로 초기 렌더 모두 `hulao-arcade-bg-v3.png` 표시. 적벽은 절차 배경 유지(회귀 없음) |
| 콘솔 에러 | 0 | 전 실행 |

코드 확인: 리사이즈 가드 `stageKey !== 'hulao'`(`sideScroller.js:1168`) ✓ · 차지샷 ×1.6·관통+1(`:1706`, `:1462`) ✓ · W 더블탭 220ms ✓(코드만) · `onPointer` 완전 제거 ✓.
미검증: 터치 활 버튼 단독(공격 버튼과 동일 핸들러) · 코덱스가 말한 "입력 경계 테스트 7개"는 저장소·package.json 어디에도 없어 재현 불가.
발견: 로컬 `main`에 동일 내용의 M1 커밋 `9f1ba8e`가 별 부모로 중복 존재 + 미커밋 파일 1개(다른 세션 작업). **푸시는 이 브랜치로만**, 로컬 main은 건드리지 말 것.
