...
(async()=>{const out=fs.mkdtempSync(path.join(os.tmpdir(),'princess-picnic-')),b=await chromium.launch({channel:'msedge',headless:true});try{
 for(const viewport of [{width:390,height:844},{width:820,height:1180},{width:1180,height:820}]){
  ...
  await dialog.getByRole('button',{name:'태오',exact:true}).click();await dialog.getByRole('heading',{name:'토끼에게 온 소풍 편지'}).waitFor();
  assert.equal(await p.evaluate(()=>PrincessGrowth.read(localStorage).children.taeo.friend),false);
  await dialog.getByRole('button',{name:'재이',exact:true}).click();await dialog.getByRole('heading',{name:'첫 친구, 토끼',exact:true}).waitFor();
 ...
 await p.reload();await p.locator('#princessGrowthHub').getByRole('button',{name:'태오',exact:true}).click();
 ...
 await p.getByRole('button',{name:'숲속 소풍 · 공주 키우기'}).click();assert.equal(await p.locator('.pj-profile').getByRole('button',{name:'태오',exact:true}).getAttribute('aria-pressed'),'true');
 ...
 await p.getByRole('button',{name:'지혜 도우미 · 두 개씩 나누기'}).click();assert.deepEqual(await p.evaluate(()=>PrincessGrowth.read(localStorage).children.taeo.quest.plates),[2,2,2]);
