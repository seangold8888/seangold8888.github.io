...
function env(){const c={location:{search:'?heads=natural'}};vm.createContext(c);for(const file of ['wardrobe.js','footwear.js','salon.js','studio.js'])vm.runInContext(fs.readFileSync(path.join(root,file),'utf8'),c);return c;}
...
test('three stable hairstyle IDs preserve old saves, invalid values fall back to base',()=>{
 ...
 const html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert(html.indexOf('salon.js?v=1')<html.indexOf('studio.js?v=49'));assert(html.includes("PrincessSalon?.has(p.id+'-half')"));
});
