...
3. `new Audio(...)`에 `loop = true`, `volume = 0.7`(게임마다 0.62~0.66로 조정, 2026-09-20 "소리가 작다" 지적으로 0.3대에서 올림), `preload = "none"`. 화면이 가려지면(`visibilitychange`) 멈추고 돌아오면 잇는다.
...
## 2026-09-25 구글 클라우드(Vertex)로 옮김

제미나이 API 선불 크레딧이 끝나(429 "prepayment credits are depleted") 새 곡은 Vertex에서 만든다. 인증은 gcloud ADC, 크레딧 프로젝트 `project-b6386b67-b505-47c8-9ac`, API 키 금지.

- 도구: `tools/vertex-lyria.py <출력.wav> "<프롬프트>" ["<빼고 싶은 것>"] [시드]`
- Vertex에는 **Lyria 3.5가 없다**(lyria-3.5·lyria-3 모두 404). 쓸 수 있는 것은 `lyria-002`(us-central1): 한 번에 약 32초, 48kHz 스테레오 WAV, 연주곡 전용, 곡당 약 $0.06.
- 기존 16곡(Lyria 3.5)과 결이 다를 수 있다. 새 곡을 넣기 전에 기존 곡과 나란히 들어 보고 고른다. 반복곡 길이는 30초대라 `loopify-bgm.py`의 40초 기준 대신 약 28초 반복으로 다듬는다.
- `tools/lyria-bgm.cjs`(제미나이 API)는 크레딧을 다시 채우면 쓸 수 있게 남겨 둔다.
