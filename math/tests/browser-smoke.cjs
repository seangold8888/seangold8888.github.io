...
const root=path.resolve(__dirname,'../..'),out=path.resolve(root,'../math-garden-qa');
...
(async()=>{
 ...
 try {
  ...
  await page.locator('.seed:not([disabled])').first().click();
  ...
  await page.evaluate(()=>{const s=MathStore.defaults();s.name='재이';s.level=9;s.placed=true;s.perSession=8;s.coins=123;s.owned={'dress/party':true};s.album=[{id:'elsa',date:'2026-09-07',r:1}];localStorage.setItem(MathStore.KEY,JSON.stringify(s));});
  await page.goto(base+'/math/');assert.equal((await state()).coins,123);assert.ok((await state()).owned['dress/party']);
  ...
  assert.deepEqual(errors,[]);console.log(JSON.stringify({ok:true,checks:'onboarding, 3-question completion, progressive hints, seed interaction, reload/resume, rewards, wardrobe, parent, migration, transfer, diagnosis',screenshots:out}));
