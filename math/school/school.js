...
let data={version:1,session:null,history:[],covered:[]},storage=null;
try{
 storage=window.localStorage;const raw=JSON.parse(storage.getItem(KEY)||"null");
 if(raw&&raw.version===1){
  data.covered=Array.isArray(raw.covered)?raw.covered.filter(t=>Number.isInteger(t)&&t>=0&&t<20):[];
...
if(data.session)data.covered=Array.from(new Set(data.covered.concat(data.session.results.map(r=>r.type))));
...
function visual(q){window.SchoolView.visual(q,$("visual"));}
function readDraft(){
...
function draw(){
 ...
 window.SchoolView.answers(q,s.draft,!!s.checked,box);
...
function record(correct){
 ...
 data.covered=Array.from(new Set(data.covered.concat(s.order[s.index])));
